...
Page({
  ...
  onLoad: function (options) {
    app.onLogin(userInfo => {
      ...
      // 获取搜索历史记录
      app.storage.getItem('stock_in_history').then(list => {
        this.setData({
          'filter.history': list || []
        })
      })
  ...
  getList: function (page = 1, callback = app.noopFn) {
    ...
    app.post(app.config.stockInList, {
  ...
  clearHistory: function () {
    app.storage.removeItem('stock_in_history')
    this.setData({
      'filter.history': []
    })
  },
  ...
  search: function () {
    ...
    if (this.data.filter.data.storageCode.trim() && !this.data.filter.history.includes(this.data.filter.data.storageCode)) {
      ...
      app.storage.setItem('stock_in_history', historyData)
    }
    this.hideFilter()
    this.getList()
  }
